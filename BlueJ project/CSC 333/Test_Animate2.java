...
public class Test_Animate2 extends Base
{
    ...
    public void update()
    {
        ...
        Math.sin(time/a) * (Math.exp(Math.cos(time/a)) - 2*Math.cos(4*time/a) - Math.pow( Math.sin((time/a)/12), 5) ), 
        Math.cos(time/a) * (Math.exp(Math.cos(time/a)) - 2*Math.cos(4*time/a) - Math.pow( Math.sin((time/a)/12), 5) ), 
        0 );
        
        position.multiplyScalar(0.25);
